...
$oExportPdf = new grid_postagem_export_pdf;
$oExportPdf->exportPdf();

class grid_postagem_export_pdf {
...
<head>
 <title><?php echo $Nm_lang['lang_othr_grid_titl'] ?> - postagem :: PDF</title>
 ...
 function checkPDF() {
  $.get(nm_url_rand("grid_postagem_gauge_ctrl.php"), {
...
<?php
$NM_pdfbase = (isset($_SESSION['sc_session'][$aParams['script_case_init']]['grid_postagem']['pdf_name'])) ? $_SESSION['sc_session'][$aParams['script_case_init']]['grid_postagem']['pdf_name'] : 'sc_pdf_' . date('YmdHis') . '_' . rand(0, 1000) . '_grid_postagem.pdf';
$NM_tit_doc = (isset($_SESSION['sc_session'][$aParams['script_case_init']]['grid_postagem']['pdf_name'])) ? $_SESSION['sc_session'][$aParams['script_case_init']]['grid_postagem']['pdf_name'] : "grid_postagem.pdf";
$NM_pdfurl  = $STR_tmp;
$NM_target  = "_self";
$NM_volta   = "volta_grid";
?>
<form name="F0" method="post" action="grid_postagem.php" target="<?php echo $NM_target; ?>" style="display: none"> 
<input type="hidden" name="nmgp_opcao" value="<?php echo $NM_volta; ?>"> 
<input type="hidden" name="script_case_init" value="<?php echo $aParams['script_case_init']; ?>"> 
<input type="hidden" name="script_case_session" value="<?php echo session_id(); ?>"> 
</form>
<form name="Fdown" method="get" action="grid_postagem_download.php" target="_blank" style="display: none"> 
<input type="hidden" name="nm_tit_doc" value="<?php echo $NM_tit_doc; ?>"> 
<input type="hidden" name="nm_name_doc" value="<?php echo $NM_pdfurl . "/" . $NM_pdfbase;?>"> 
</form>
...
{
?>
<iframe name="nmIfrPdf" height="1px" width="1px" frameborder="0" scrolling="no" allowtransparency="true" src="grid_postagem.php?nmgp_opcao=pdf&script_case_init=<?php echo $aParams['script_case_init'] ?>&script_case_session=<?php echo session_id() ?>&pbfile=<?php echo $aParams['pbfile'] ?>&pdf_base=<?php echo $NM_pdfbase ?>&pdf_url=<?php echo $NM_pdfurl ?>&sc_apbgcol=<?php echo urlencode($aParams['sc_apbgcol']) ?>&nmgp_tipo_pdf=<?php echo $aParams['sc_tp_pdf'] ?>&nmgp_parms_pdf=<?php echo $parms_pdf ?>&nmgp_graf_pdf=<?php echo $graf_pdf ?>&flash_graf=chart"></iframe>
<?php
}
else
{
?>
<iframe name="nmIfrPdf" height="1px" width="1px" frameborder="0" scrolling="no" allowtransparency="true" src="grid_postagem.php?nmgp_opcao=pdf_res&script_case_init=<?php echo $aParams['script_case_init'] ?>&script_case_session=<?php echo session_id() ?>&pbfile=<?php echo $aParams['pbfile'] ?>&pdf_base=<?php echo $NM_pdfbase ?>&pdf_url=<?php echo $NM_pdfurl ?>&sc_apbgcol=<?php echo urlencode($aParams['sc_apbgcol']) ?>&nmgp_tipo_pdf=<?php echo $aParams['sc_tp_pdf'] ?>&nmgp_parms_pdf=<?php echo $parms_pdf ?>&nmgp_graf_pdf=<?php echo $graf_pdf ?>&flash_graf=chart"></iframe>
<?php
}
?>
   </td>
