...
class nmConnection
{
    ...
    {    	
        ...
        else
        {
            ...
            if ('ado_access' == $str_dbms)
            {
                $str_host = 'PROVIDER=Microsoft.Jet.OLEDB.4.0;'
                          . 'DATA SOURCE=' . $str_host . ';';
                $str_user = '';
                $str_pass = '';
                $str_base = '';
            }
            elseif ('ado_mssql' == $str_dbms)
            {
            ...
            elseif ('pdo_informix' == $str_dbms)
            {
		        ...
		        }else 
		        {
			    	...
			    	{
			    		list($str_host, $str_port) = explode(":", $str_host);
			    	}
		        	if(strpos($str_host, "\\") !== false)
			    	{
			    		list($str_host, $str_server) = explode("\\", $str_host);
			    	}
            ...
            elseif ('pdo_mysql' == $str_dbms)
            {
				...
				if(strpos($str_host, ":") !== false)
				{
					list($str_host, $port) = explode(":", $str_host);
				}
            ...
            elseif ('db2_odbc' == $str_dbms)
            {
                $str_host  = "driver={IBM db2 odbc DRIVER};Database=". $str_base .";hostname=". $str_host .";port=50000;protocol=TCPIP;";
    ...
    {    	
        ...
        nm_load_class('xmlparser', 'XmlparserConnection');
        $xml_conn =& new nmXmlparserConnection();
		...
		$_SESSION['nm_session']['cache']['prod'] = $arr_ini;
		
		file_put_contents($prod_ini_file, '<?php /*' . $xml_conn->GetXml($arr_ini) . '*/ ?>');
    }//SaveConn
